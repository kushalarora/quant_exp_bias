...
for i in range(0, len(sequences), batch_size):
    ...
    with torch.no_grad():
        ...
        for i in range(3, seq_len): 
            logits, past =  model(prefixes, past=past)
            _, predictions = torch.topk(logits[:, -1, :], k=1)
            # predicted_token = tokenizer.convert_ids_to_tokens(predictions.tolist())
            prefixes = torch.cat([prefixes, predictions], dim=1)
        
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = prefixes[..., 1:].contiguous()
