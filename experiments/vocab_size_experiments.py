...
vocabulary_sizes  = [3, 12, 24, 48, 90]
vocab_distributions = ['zipf', 'uniform']
grammar_template = 'grammar_templates/grammar_1.template'

num_sample_oracles = 4 
num_trials = 10
num_samples_per_length=2000

# # Validation Experiments
vocab_size_and_dist = [x for x in itertools.product(vocabulary_sizes, vocab_distributions)]
...
def vocabulary_size_experiments(main_args, serialization_dir, param_path):
    ...
    for size, dist in vocab_size_and_dist:
        vocab_size_exp_results[f'{dist}_{size}'] = []
    ...
    for size, dist in vocab_size_and_dist:
        ...
        for num_run in range(num_sample_oracles):
            serialization_dir = os.path.join(vsexp_serialization_dir, str(num_run))
            sample_oracle_args = get_args(args=['sample-oracle', param_path, '-s', serialization_dir, '-o', overrides, '-n', '50000'])
